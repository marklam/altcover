...
module AltCoverTests2 =
    ...
    let ShouldBeAbleToGetTheVisitReportMethod() =
      ...
      let path =
        Path.Combine
          (Path.GetDirectoryName(where) + Hack(), "AltCover.Recorder.dll")
    ...
    let ShouldGetNewFilePathFromPreparedAssembly () =
      try
        ...
        Main.init()
        let where = Assembly.GetExecutingAssembly().Location
        let path = Path.Combine(Path.GetDirectoryName(where) + Hack(), "Sample3.dll")
...
// TODO -- see Instrument.WriteAssembly       Assert.That (File.Exists (outputdll + ".mdb"))
    ...
    let ShouldBeAbleToTrackAMethod() =
      ...
      let path =
        Path.Combine
          (Path.GetDirectoryName(where) + Hack() + shift,
